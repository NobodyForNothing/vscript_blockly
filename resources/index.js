...
function createVscriptGenerator(workspace) {
    const vscriptGenerator = new Blockly.Generator('Vscript');
    
    // variable names todo: understand implementation
    defvars = []
    if (!vscriptGenerator.nameDB_) {
      vscriptGenerator.nameDB_ = new Blockly.Names(vscriptGenerator.RESERVED_WORDS_);
    } else {
      vscriptGenerator.nameDB_.reset();
    }
    ...
    function idToName(id) { // todo find cleaner implementation
    ...
    vscriptGenerator['controls_for'] = function(block) {
      const varName = idToName(block.getFieldValue('VAR')); 
      console.log(varName);
...
const vscriptGenerator = createVscriptGenerator(workspace);
